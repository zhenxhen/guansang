...
const WebcamDetection: React.FC<WebcamDetectionProps> = ({ userName }) => {
  ...
  const handleTestVersionConfirm = () => {
    if (modalStep === 1) {
      // 첫 번째 단계에서 Next 버튼을 누르면 두 번째 단계로
      setModalStep(2);
    } else {
    ...
    <>
      ...
      >
        ...
        >
          {/* 컨테이너 블러 오버레이 */}
          {isBlurred && (
            <div className={`container-blur-overlay ${!isBlurred ? 'hidden' : ''}`}></div>
          )}
        ...
        {result && (
          <div 
            className={`collapsible-section ${!sectionsVisible ? 'hidden-section' : ''}`} 
            style={{ 
              bottom: graphExpanded ? '350px' : '70px', 
              zIndex: 2000
            }}
          >
            <h4 
